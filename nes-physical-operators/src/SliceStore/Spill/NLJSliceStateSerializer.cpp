...
NLJSliceStateSerializer::spill(Slice& slice, StorageBackend& backend, AbstractBufferProvider& /*buffers*/)
{
    ...
    auto spillSide = [&](SpillRole role, uint64_t numVectors, auto getPagedVector) -> std::expected<void, IoError>
    {
        ...
        {
            const TupleBuffer* pvBuffer = getPagedVector(WorkerThreadId{static_cast<uint32_t>(threadIdx)});
            if (pvBuffer == nullptr)
            {
                continue;
            }
            auto pagedVector = PagedVector::load(*pvBuffer);
            auto pages = pagedVector.drainPages();
            ...
            /// Successful drain: the page handles are dropped here, which returns their memory to the pool --
            /// drainPages detached them from the PagedVector's control buffer, so this was the last reference.
        }
        return {};
    };
...
NLJSliceStateSerializer::restore(Slice& slice, const SpilledSliceHandle& handle, StorageBackend& backend, AbstractBufferProvider& buffers)
{
    ...
    {
        if (const auto* pvBuffer = nlj.getPagedVectorRefLeft(WorkerThreadId{static_cast<uint32_t>(threadIdx)}); pvBuffer != nullptr)
        {
            PagedVector::load(*pvBuffer).adoptPages(std::move(leftByThread[threadIdx]));
        }
    }
    for (uint64_t threadIdx = 0; threadIdx < rightByThread.size(); ++threadIdx)
    {
        if (const auto* pvBuffer = nlj.getPagedVectorRefRight(WorkerThreadId{static_cast<uint32_t>(threadIdx)}); pvBuffer != nullptr)
        {
            PagedVector::load(*pvBuffer).adoptPages(std::move(rightByThread[threadIdx]));
        }
    }
...
uint64_t NLJSliceStateSerializer::residentBytes(const Slice& slice) const noexcept
{
    ...
    for (uint64_t threadIdx = 0; threadIdx < nlj.getNumberOfLeftPagedVectors(); ++threadIdx)
    {
        if (const auto* pvBuffer = nlj.getPagedVectorRefLeft(WorkerThreadId{static_cast<uint32_t>(threadIdx)}); pvBuffer != nullptr)
        {
            const auto pagedVector = PagedVector::load(*pvBuffer);
            total += pagedVector.getNumberOfPages() * pagedVector.getPageBufferSize();
        }
    }

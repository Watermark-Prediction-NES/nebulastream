...
#include <SliceStore/Spill/SliceEvictionConfiguration.hpp>
    ...
    AbstractBufferProvider* bufferProvider)
{
    ...
    auto policy = SpillPolicyRegistry::instance()
                      .create(
                          evictionConfig.policyName,
                          SpillPolicyRegistryArguments{
                              .lowMemoryBound = evictionConfig.lowMemoryBound,
    ...
    if (!policy)
    {
        NES_WARNING("SliceStoreFactory: unknown spill policy '{}', falling back to never-spill", evictionConfig.policyName);
    ...
    if (!backend)
    {
        NES_WARNING("SliceStoreFactory: unknown storage backend '{}', falling back to in-memory", evictionConfig.storageBackendName);

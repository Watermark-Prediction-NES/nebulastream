    ...
    const Timestamp minTs, const Timestamp maxTs, const double watermarkRate, const SliceCreateFunction& createNewSlice)
{
    ...
    const auto spanSlices = ((maxTs.getRawValue() - minTs.getRawValue()) / sliceAssigner.getWindowSlide()) + 1;
    ...
    uint64_t missingSlices = 0;
    {
        ...
        for (auto ts = minTs; ts <= maxTs; ts = sliceAssigner.getSliceEndTs(ts))
        {
    ...
    uint64_t claimId = 0;
    {
        ...
        {
            if (range.rangeStart <= maxTs and minTs <= range.rangeEnd)
            {
    ...
    for (auto ts = minTs; ts <= claimMaxTs; ts = sliceAssigner.getSliceEndTs(ts))
    {

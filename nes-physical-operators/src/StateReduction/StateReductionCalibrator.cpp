...
StageCost fitLine(const std::vector<Sample>& samples)
{
    ...
    /// A negative coefficient is measurement noise, not a real effect, and it would let the model
    /// predict that bigger states are cheaper.
    cost.nanosPerByte = std::max(cost.nanosPerByte, 0.0);

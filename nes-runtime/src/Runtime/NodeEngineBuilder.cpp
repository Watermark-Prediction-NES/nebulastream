...
{
    const auto& bufferUsageLogPath = workerConfiguration.bufferUsageLogPath.getValue();
    auto bufferManager = BufferManager::create(
        ...
        bufferUsageLogPath.empty() ? std::nullopt : std::optional{std::filesystem::path{bufferUsageLogPath}},

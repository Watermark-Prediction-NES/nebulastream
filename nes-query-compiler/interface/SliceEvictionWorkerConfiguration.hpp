...
///
/// The POD SliceEvictionConfiguration stays the per-query lingua franca (it flows through the SQL binder and
/// logical plan as a plain copy). This BaseConfiguration is only the engine-default surface: the
/// QueryCompiler converts it via toSliceEvictionConfiguration() and a per-query `SET (EVICTION.* AS ...)` clause

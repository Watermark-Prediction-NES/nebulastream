...
{
    EncodedMap result;
    auto mainBuffer = bufferProvider.getUnpooledBuffer(ChainedHashMap::calculateBufferSizeFromBuckets(NUMBER_OF_BUCKETS));
    EXPECT_TRUE(mainBuffer.has_value());
    ChainedHashMap::init(mainBuffer.value(), KEY_SIZE, VALUE_SIZE, NUMBER_OF_BUCKETS, PAGE_SIZE);

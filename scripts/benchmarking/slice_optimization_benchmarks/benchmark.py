...
QUERIES = {
    ...
    ### Lighter variants over the 1GB google-cluster trace (vs the 6GB bid file) — same 1 second slide,
    ### far fewer tuples/slices, so the sweep fits a small Docker VM. Subset with e.g. `-q CM_AGG CM_JOIN`.
    "CM_AGG": "nes-systests/benchmark/memory-source/SliceOptimization_memory.test:03",
    "CM_JOIN": "nes-systests/benchmark/memory-source/SliceOptimization_memory.test:04",
}
...
PREEMPTIVE_PREDICTORS = ["off", "robustkalman"]
...
### Default worker memory config — large (1MB) buffers keep cells fast. The worker takes one total
### budget and splits it into a pooled share (operator buffers, count = total x (1 - fraction) /
### buf_size) and an unpooled share (var-sized data, hash maps, paged vectors). Unpooled state used
### to sit unbounded on top of the pool; it now comes out of the same budget.
BUFFER_SIZE_BYTES = 1_048_576
TOTAL_MEMORY = 40 * 1024**3
UNPOOLED_FRACTION = 0.5
PAGE_SIZE = 8192

### Per-query overrides: (operator_buffer_size, total_memory_in_bytes). The heaviest varsized
### windowed Nexmark queries need finer-grained allocation, so give them 128KiB buffers instead of
### the fast 1MB default.
BUFFER_OVERRIDES = {
    "NM8_Variant": (131_072, TOTAL_MEMORY),
    "NM8": (131_072, TOTAL_MEMORY),
    ### Thousands of live slices make these the heaviest queries in the suite. The budget is
    ### allocated up front, so 4GiB fits a ~7.7GiB Docker VM. The no-spill baseline cells
    ### (predictor=off) genuinely can't hold ~23k slices in this budget; they fail with
    ### pool-exhaustion (recorded, not OOM-killed). For the full in-memory baseline, run on a
    ### big-RAM box and raise these back to TOTAL_MEMORY.
    "SLICE_AGG": (65_536, 4 * 1024**3),
    "SLICE_JOIN": (65_536, 4 * 1024**3),
    ### 1GB-cluster variants: smaller windows -> fewer live slices, so 2GiB is plenty.
    "CM_AGG": (65_536, 2 * 1024**3),
    "CM_JOIN": (65_536, 2 * 1024**3),
}
...
def spill_set_clause(predictor: str, high_bound: float | None = None) -> str:
    ...
    bound = f", {high_bound} AS EVICTION.HIGH_BOUND" if high_bound is not None else ""
    return (
        f"SET (TRUE AS EVICTION.ENABLED, 'predictive' AS EVICTION.POLICY, "
        f"'in-memory' AS EVICTION.BACKEND, '{predictor}' AS EVICTION.PREDICTOR{bound})"
    )
...
def run_cell(qname: str, query_test: str, horizon: int, preemptive_predictor: str, max_slices: int,
    ...
    buf_size, total_memory = BUFFER_OVERRIDES.get(qname, (BUFFER_SIZE_BYTES, TOTAL_MEMORY))
    worker_cfg = " ".join([
        ...
        f"--worker.unpooled_memory_fraction={UNPOOLED_FRACTION}",
...
def parse_argv(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--high-bound", type=float,
                   help="EVICTION.HIGH_BOUND override; >1.0 guarantees never-spill (predictor overhead only)")

...
BUFFER_OVERRIDES = {
    ...
    "NM8": (131_072, 320_000),
    ### Thousands of live slices make these the heaviest queries in the suite. Global pool is
    ### preallocated as buf_size x num_buffers, so 64KiB x 65536 ~= 4GiB fits a ~7.7GiB Docker VM.
    ### The no-spill baseline cells (predictor=off) genuinely can't hold ~23k slices in this budget;
    ### they fail with pool-exhaustion (recorded, not OOM-killed). For the full in-memory baseline,
    ### run on a big-RAM box and raise these back to (131_072, 320_000).
    "SLICE_AGG": (65_536, 65_536),
    "SLICE_JOIN": (65_536, 65_536),
    ### 1GB-cluster variants: smaller windows -> fewer live slices, so a ~2GiB pool is plenty.
    "CM_AGG": (65_536, 32_768),
    "CM_JOIN": (65_536, 32_768),
}

...
### Bench harness for the slice-store optimizations:
###   --worker.default_query_execution.slice_store.enable_slice_recycling
###   --worker.default_query_execution.slice_store.enable_slice_group_creation (+ max_slice_group_size)
### Invoked from the nebulastream repo root; the local source tree is mounted into the dev docker
### image at /tmp/nebulastream and benchmark.py runs inside the container.
set -euo pipefail
...
BENCH_REL="scripts/benchmarking/slice_recycling_benchmarks"
...
docker run --rm ${DOCKER_TTY_FLAGS[@]+"${DOCKER_TTY_FLAGS[@]}"} \
    ...
    -e BENCH_BUILD_DIR_OVERRIDE="${BENCH_BUILD_DIR_OVERRIDE:-}" \
    -v ccache:/ccache \

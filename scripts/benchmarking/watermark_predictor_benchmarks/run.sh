...
if [[ ! -f "./CMakeLists.txt" || ! -d "./nes-common" || ! -d "./nes-physical-operators" ]]; then
    echo "error: run.sh must be invoked from the nebulastream repo root." >&2
    echo "       cwd=$(pwd)" >&2
    echo "       expected to find ./CMakeLists.txt + ./nes-common/ + ./nes-physical-operators/" >&2
    exit 2
fi
...
BENCH_REL="scripts/benchmarking/watermark_predictor_benchmarks"
...
### -it auto-suppressed when stdin/stdout aren't ttys.
DOCKER_TTY_FLAGS=()
if [[ -t 0 && -t 1 ]]; then
    DOCKER_TTY_FLAGS+=(-it)
fi

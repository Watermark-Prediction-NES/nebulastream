...
@test "worker rejects an unknown state_reduction forced_decision" {
  ...
  run timeout 5 $NES_WORKER --worker.default_query_execution.state_reduction.forced_decision=KEEP_IN_MEMORY
...
@test "worker rejects state_reduction learning_rate out of range" {
  ...
  run timeout 5 $NES_WORKER --worker.default_query_execution.state_reduction.learning_rate=1.5
  grep -E "invalid config parameter|Validator" singleNodeWorker.log
  grep "learning_rate" singleNodeWorker.log

  run timeout 5 $NES_WORKER --worker.default_query_execution.state_reduction.learning_rate=-0.1
  grep -E "invalid config parameter|Validator" singleNodeWorker.log
  grep "learning_rate" singleNodeWorker.log
}
